...
fn basic_incremental_use() {
    ...
    for negs in [vec![x],vec![y],vec![z],vec![x,y],vec![x,z],vec![y,z]].iter() {
        println!("solving {:?} v {:?} v {:?}  and neg {:?}", x,y,z,negs);
        s.prop.set_assumptions(negs.iter().map(|l| l.inverse()));
        assert!(s.prop.solve() == LBOOL_TRUE);
        let model = s.prop.get_model().unwrap().iter().map(|l| *l == LBOOL_TRUE).collect::<Vec<bool>>();
        println!("model: {:?}", model);
    ...
    s.prop.set_assumptions(vec![x,y,z].iter().map(|l| l.inverse()));
