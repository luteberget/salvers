...
pub trait BooleanFormulas<L: Lit> {
    ...
    fn assert_parity_or(&mut self, prefix :impl IntoIterator<Item = impl Into<Bool<L>>>, xs: impl IntoIterator<Item = impl Into<Bool<L>>>, c :bool);
}
...
impl<L: Lit, S: SatInstance<L>> BooleanFormulas<L> for S {
    ...
    fn xor_literal(&mut self, xs: impl IntoIterator<Item = impl Into<Bool<L>>>) -> Bool<L> {
          ...
          let out = posneg[0]
              ...
              .chain(posneg[1].iter().map(|x| Bool::Lit(Lit::from_var_sign(*x, true))))
              .collect::<Vec<_>>();
          if out.len() == 0 {
              const_parity.into()
          } else if out.len() == 1 {
